...
async def bpm_hour_feature(user_email: str, start_date: str, end_date: str):
    ...
    # query = bpm_test2.find({'user_email': user_email, 'timestamp': {'$gte': datetime.fromtimestamp(int(str(start_date)[:-3])), '$lte': datetime.fromtimestamp(int(str(end_date)[:-3]))}})
...
async def get_bpm_all_data(user_email):
    cursor = bpm_test2.find({'user_email': user_email})
    results = []
    async for document in cursor:
        results.append(document)
    return results
...
async def bpm_minute_predict(user_email: str):
    # query = bpm_div.find({'user_email': user_email})
    # query = bpm_test2.find({'user_email': user_email})
    query = await get_bpm_all_data(user_email)
    # list_query = list(query)
    # mongo_bpm_df = pd.DataFrame({
    #     'ds': [list_query[x]['timestamp'] for x in range(len(list_query))],
